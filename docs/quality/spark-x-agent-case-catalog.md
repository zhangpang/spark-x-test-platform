...
`CONV-001/002`、`CHAT-001/002`、`TOOL-001/002`、`KB-001`、`SKILL-001` 与 `AUTO-001` 已具备用例定义和受信任适配器执行闭环。CONV-001 覆盖创建会话、
...
`scripts/provision-spark-x-agent-conversation-p0.ts` 幂等创建；脚本同时维护 CONV、CHAT、TOOL、KB、SKILL、AUTO 诊断套件和当前含九条
...
中已有的引用值，适合发布后无人值守回归。
当前进度为核心冒烟 9/10～12；仍以跨七个模块的 P0 全部落地为完成条件。
