...
字段：锁键、运行/用例迭代、租约截止时间、心跳时间和释放原因。活动锁键唯一；多锁按规范化键排序并在同一事务中获取，过期租约保留历史后释放。
...
Worker 中断或常规清理失败后的补偿任务。它独立于原测试运行，保存原运行汇总、门禁结论、首次失败、重试次数和最终处理结果；实际待清理资源始终来自 `resource_ledger`，不在队列消息中复制。
