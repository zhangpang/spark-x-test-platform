    ...
    const automationDefinition = definition({
      ...
      steps: [
        ...
        },
        {
          ...
          capture: {
            "last-fire-at": "$.lastFireAt",
            "next-fire-at": "$.nextFireAt",
            "assistant-sha256": "$.assistantContentSha256",
          },
        },
        {
          id: "assert-no-duplicate-delivery",
          name: "assert no duplicate delivery",
          kind: "action",
          ...
          params: {
            ...
            expectedGoal: "reply only with spark-x-auto-${run.id}",
            expectedAssistantText: "spark-x-auto-${run.id}",
            expectedLastFireAt: "${step.last-fire-at}",
            expectedNextFireAt: "${step.next-fire-at}",
            expectedAssistantSha256: "${step.assistant-sha256}",
          },
        },
      ],
